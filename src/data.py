...
class Dataset():
    ...
    def __init__(self, cfg: dict):
        ...
        forcing_list = ['P', 'Q', 'sp', 'ssrd',
                        'strd', 't2m', 'u10', 'v10', 'swvl1', 'swvl2']
    ...
    def fit(self):
        ...
        # NOTE: We used 2000-2020 modis LAI (Yuan et al.) climatology mean
        #       as a static variables
        ...
        if os.path.exists(self.inputs_path+file_name):
            ...
            static = self._load_ancillary(
                self.data_path+"ancillary/", self.s_resolution)
        ...
        if self.crop:
            print(
                '[DataML] start crop AMAZON, more flexiable crop mode will comes later!')
    ...
    def _load_et(self, et_root, et_product, temporal_resolution, spatial_resolution):
        l = glob.glob(et_root + "*{name}_{tr}_{sr}*nc".format(
            name=et_product, tr=temporal_resolution, sr=spatial_resolution))
    ...
    def _normalize(self, feature, variable, scaler, scaler_type):
        if scaler_type == 'standard':
            if variable == 'input':
                feature = (
                    feature - np.array(scaler["input_mean"])) / np.array(scaler["input_std"])
            elif variable == 'output':
                feature = (
                    feature - np.array(scaler["output_mean"])) / np.array(scaler["output_std"])
            else:
                raise RuntimeError(f"Unknown variable type {variable}")
        elif scaler_type == 'minmax':
            if variable == 'input':
                feature = (feature - np.array(scaler["input_mean"])) / (
                    np.array(scaler["input_std"])-np.array(scaler["input_mean"]))
            elif variable == 'output':
                feature = (feature - np.array(scaler["output_mean"])) / (
                    np.array(scaler["output_std"])-np.array(scaler["output_mean"]))
            else:
                raise RuntimeError(f"Unknown variable type {variable}")
        return feature
    ...
    def _get_minmax_scaler(self, X, y, type: str) -> dict:
        ...
        if type == 'global':
            scaler["input_mean"] = np.nanmin(
                X, axis=(0, 1), keepdims=True).tolist()
            scaler["input_std"] = np.nanmax(
                X, axis=(0, 1), keepdims=True).tolist()
            scaler["output_mean"] = np.nanmin(
                y, axis=(0, 1), keepdims=True).tolist()
